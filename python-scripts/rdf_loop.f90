...
! This program computes the RDF of a
! polymer melt. Distances are computed using the
! center-of-mass of the central chain and the center-of-mass
! of the other chains in the system. The speed of this
! code could be improved by working with only the
! center-of-mass positions and using the minimum image
! convention instead of translating the central cell.
...
! Lastly, be sure to set character(9) chrf and character(13) flnm
...
real*8 rsq, rij
real*8 xcm, ycm, zcm, xcmj, ycmj, zcmj
...
do f = istr, iend, incr
  ...
  write(*,*) "File ", flnm, " is ", (files + 1), " of ", (iend - istr) / incr
  ...
  do i = 1, chains
    ...
    if(xcm.lt.0.0) ix = ix - 1
    if(ycm.lt.0.0) iy = iy - 1
    if(zcm.lt.0.0) iz = iz - 1
  ...
  do i = 1, chains
    ...
    ! loop over all chains except for i
    do j = 1, chains27
      ...
      if(i.ne.j) then
        ...
        rsq = (xcm - xcmj)**2 + (ycm - ycmj)**2 + (zcm - zcmj)**2
        if(rsq.le.L2sq) then
          rij = sqrt(rsq)
          ibin = int(rij / dr) + 1
